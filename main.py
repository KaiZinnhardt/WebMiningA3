...
def iterative_response_generation(prompt):
    ...
    #To disable the chat input such that it is necessary to select one of the three choices
...
def create_final_story():
    ...
    message = [
        {"role": "system",
         "content": "You are a helpful assistant that replies in a tone for a 4 year old kid, for which you create a bedtime story."},
        {"role": "user",
         "content": "Create a creative, engaging, and extensive bedtime story, with a maximum of 1500 words, for a child while using the following summary: " + st.session_state.text_summary}
    ]
    # calculate the allowed tokens
    allowed_tokens = Helper.max_token_calculator(message)
    # checks if the message is senitized
    ...
    # creates the prompt of the image. Thereby, we use a summary of the response to create a cost efficient dialogue
    image_prompt = "Create an image in a cartoon like style for the following context:" + Helper.summarize_text_for_image(
        '', response_text, 5, 2)
    # checks if the message is senitized
    mod_bool = moderation_check(image_prompt)
    # if it is senitized it should not give an answer
...
def write_response_prompt(prompt,button):
    """
    Writes the response to the prompt of the fairytale
    :param prompt: Gives the prompt of the fairytale
    """
    #checks if the prompt has and input
    if prompt:
        if button == False:
            ...
            if Helper.token_counts(prompt) > 1500:
                st.write("The input is limited to 1500 tokens")
                st.session_state.messages.append({"role": "assistant", "content": "The input is limited to 1500 tokens"})
            with st.chat_message("assistant"):
            ...
            st.session_state.messages.append({"role": "user", "content": prompt})
            if ('complete the story' not in prompt.lower()) or (len(prompt) > len('complete the story')):
                st.write("You can only complete the story with a button")
            else:
                complete_story()
            # Rerun the Application to properly end the story
            st.rerun()
...
def main():
    ...
    for i, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"]):
            ...
            if i == len(st.session_state.messages) - 1:
                ...
                #Show a button to end the story
                if message["role"] == "assistant" and "option1" not in message and st.session_state.complete_story == False and st.session_state.text_summary != '':
                    st.markdown("\n")
                    # If pressed complete the story
                    if st.button("Complete the Story", key=button5_key, disabled=(not st.session_state.toggle)):
                        write_response_prompt("Complete the Story",True)
                    st.markdown("***Or Continue the Story by Typing the Next Prompt in the Input Field Below***")

    #Create the hint for the user how to complete the story via the input field
    ...
    # React to user input
    prompt = st.chat_input("Feed me with ideas for a Bedtime story...              (Please write 'Complete the story' to end it)", key=input1_key, disabled=(st.session_state.complete_story or toggle_input()))
    #function to respond to the prompt
    write_response_prompt(prompt,False)
